...
void  BSP_Ser_ISR_Handler (void)
{
    ...
    if (DEF_BIT_IS_SET(status, DEF_BIT_05)) {
        ...
        USART_ClearITPendingBit(USART1, USART_IT_RXNE);         /* Clear the USART3 receive interrupt.                */
        BSP_OS_SemPost(&BSP_SerRxWait);                         /* Post to the sempahore                              */
    }

    if (DEF_BIT_IS_SET(status, DEF_BIT_07)) {
        USART_ITConfig(USART1, USART_IT_TXE, DISABLE);
        USART_ClearITPendingBit(USART1, USART_IT_TXE);          /* Clear the USART3 receive interrupt.                */
...
CPU_INT08U  BSP_Ser_RdByteUnlocked (void)
{
    ...
    USART_ITConfig(USART1, USART_IT_RXNE, ENABLE);              /* Enable the Receive not emptyt interrupt            */
